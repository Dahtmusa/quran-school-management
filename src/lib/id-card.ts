...
export async function printAcademicIdCard(input:{type:'STUDENT'|'STAFF'|'MANAGEMENT';name:string;id:string;admissionNo?:string;photoUrl?:string|null;year?:string;section?:string;className?:string|null;jobTitle?:string;department?:string;phone?:string;expiry?:string|null;logoUrl?:string|null;}){
 ...
 *{box-sizing:border-box;margin:0;padding:0}
 ...
 .card{width:360px;border-radius:13px;overflow:hidden;position:relative;background:#fff;box-shadow:0 10px 28px rgba(16,37,31,.28);page-break-after:always}
 .card:last-child{page-break-after:auto}
 .band{height:50px;display:flex;align-items:center;gap:9px;padding:0 12px;background:linear-gradient(120deg,#07523f 0%,#062d2a 70%,#07241f 100%);color:#fff;border-bottom:2px solid #c9a84c;position:relative;z-index:1}
 ...
 <div class="card front">
  <div class="band">${logoMarkup}<div class="brand-badge"><strong>AMQM</strong><small>Aliyu &amp; Maimuna Center for Qur\u2019anic Memorization</small><div class="arabic">${ARABIC_SCHOOL_NAME}</div></div><div class="band-tag">${input.type==='STUDENT'?'Student<br/>ID':input.type==='MANAGEMENT'?'Management<br/>ID':'Staff<br/>ID'}</div></div>

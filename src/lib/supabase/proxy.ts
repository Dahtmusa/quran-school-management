...
const publicPaths=['/','/auth/login','/admissions','/admissions/track','/j','/alumni','/about','/programs','/campus-life','/news','/contact'];
const roleRoutes:{prefix:string;roles:string[]}[]=[
 ...
 {prefix:'/security',roles:['security','super_admin','admin','principal']},
];
function isPublic(path:string){return publicPaths.some(p=>p==='/admissions'?path==='/admissions':path===p||path.startsWith(p+'/'))}
function dashboard(role:string){if(role==='teacher')return '/teacher';if(role==='parent')return '/parent';if(role==='security')return '/attendance/scan';if(role==='finance')return '/fees';if(role==='admissions')return '/admissions/manage';return '/admin'}

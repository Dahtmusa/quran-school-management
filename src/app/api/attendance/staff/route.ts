...
export async function GET(req:NextRequest){
 ...
 const [staffRes,recordsRes,finesRes,statusRes,settingsRes]=await Promise.all([
  supabase.from('profiles').select('id,full_name,staff_id,role,employment_status').in('role',['teacher','admin','super_admin','principal','finance','security','admissions']).eq('employment_status','active').order('full_name'),

...
export default function Topbar({title}:{title:string}){
 ...
 {open&&<div className="fixed inset-0 z-[200] md:hidden" role="dialog" aria-modal="true" aria-label="AMQM navigation">
   <button aria-label="Close navigation overlay" className="absolute inset-0 bg-slate-950/45 backdrop-blur-[1px]" onClick={()=>setOpen(false)}/>
   <aside className="relative flex h-[100dvh] w-[min(88vw,360px)] max-w-full flex-col overflow-hidden bg-[#063b34] text-white shadow-[18px_0_50px_rgba(0,0,0,.28)] pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
     <div className="shrink-0 border-b border-white/10 px-5 py-5">
       <div className="flex items-center justify-between gap-4">
         <div className="min-w-0">
           <div className="text-2xl font-black tracking-tight">AMQM</div>
           <div className="mt-1 truncate text-xs font-semibold capitalize text-emerald-100/70">{me?.role||'Account'} workspace</div>
         </div>
         <button aria-label="Close navigation" onClick={()=>setOpen(false)} className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white/10 text-2xl font-light hover:bg-white/15">×</button>
       </div>
     </div>
     <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-4 overscroll-contain">
       <div className="mb-3 px-3 text-[10px] font-black uppercase tracking-[.18em] text-emerald-200/55">Navigation</div>
       <div className="space-y-1.5">
         {links.map(([href,label])=><Link onClick={()=>setOpen(false)} className={`flex min-h-12 items-center gap-3 rounded-2xl px-4 py-3 text-[15px] font-bold transition ${path===href||path.startsWith(href+'/')?'bg-white text-emerald-950 shadow-sm':'text-white hover:bg-white/10'}`} href={href} key={href}>
           <span className="min-w-0 flex-1">{label}</span><span className={path===href||path.startsWith(href+'/')?'text-emerald-700':'text-emerald-200'}>→</span>
         </Link>)}
       </div>
       {isAdmin&&<div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-4">
         <div className="text-[10px] font-black uppercase tracking-[.16em] text-emerald-200/55">Active term</div>
         <div className="mt-1 text-sm font-bold text-white">{yearLabel&&termLabel?`${yearLabel} · ${termLabel}`:'Not set'}</div>
         <Link href="/calendar" onClick={()=>setOpen(false)} className="mt-2 inline-block text-xs font-bold text-emerald-200 hover:text-white">Manage calendar →</Link>
       </div>}
     </nav>
     <div className="shrink-0 border-t border-white/10 bg-[#052f2a] px-4 py-4">
       <Link onClick={()=>setOpen(false)} href="/" className="block rounded-2xl bg-white px-4 py-3 text-center text-sm font-black text-emerald-950">View website</Link>
       <button onClick={signOut} className="mt-2 block w-full rounded-2xl bg-white/10 px-4 py-3 text-sm font-bold text-white hover:bg-white/15">Sign out</button>
     </div>
   </aside>
 </div>}

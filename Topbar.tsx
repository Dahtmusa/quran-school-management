...
export default function Topbar({title}:{title:string}){
 ...
 const links=roleLinks(normalizedRole);
 ...
 {open&&<div className="fixed inset-0 z-[100] md:hidden" role="dialog" aria-modal="true" aria-label="Mobile navigation"><button aria-label="Close navigation overlay" className="absolute inset-0 bg-slate-950/55" onClick={()=>setOpen(false)}/><aside className="absolute inset-y-0 left-0 flex w-[min(88vw,380px)] flex-col overflow-hidden bg-[#062d2a] text-white shadow-2xl"><div className="shrink-0 border-b border-white/10 p-5"><div className="flex items-center justify-between"><div><div className="text-2xl font-black tracking-tight">AMQM</div><div className="mt-1 text-xs font-semibold text-emerald-200/70">{normalizedRole?`${normalizedRole.replace('_',' ')} workspace`:'School workspace'}</div></div><button aria-label="Close navigation" onClick={()=>setOpen(false)} className="grid h-11 w-11 place-items-center rounded-xl bg-white/10 text-2xl hover:bg-white/15">×</button></div></div><div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4"><div className="mb-3 px-2 text-[10px] font-black uppercase tracking-[.18em] text-emerald-300/70">Navigation</div><nav className="space-y-1.5">{links.length?links.map(([href,label])=><Link onClick={()=>setOpen(false)} className={`flex min-h-12 items-center justify-between rounded-2xl px-4 py-3.5 text-sm font-bold ${pathname===href||pathname.startsWith(`${href}/`)?'bg-white text-emerald-950 shadow-sm':'text-emerald-50 hover:bg-white/10'}`} href={href} key={href}>{label}<span className={pathname===href||pathname.startsWith(`${href}/`)?'text-emerald-700':'text-emerald-300'}>→</span></Link>):<div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-emerald-100">Loading navigation…</div>}</nav></div><div className="shrink-0 border-t border-white/10 p-4"><Link onClick={()=>setOpen(false)} href="/" className="block rounded-2xl bg-white px-4 py-3.5 text-center text-sm font-black text-emerald-950">View website</Link><button onClick={signOut} className="mt-2 block w-full rounded-2xl bg-white/10 px-4 py-3.5 text-sm font-bold text-white hover:bg-white/15">Sign out</button></div></aside></div>}

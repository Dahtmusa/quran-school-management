...
export default function Topbar({title}:{title:string}){
 ...
 return <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl"><div className="flex min-h-16 items-center gap-3 px-3 py-2 sm:px-4 md:px-7">
   <button aria-label="Open navigation" className="mobile-menu-button md:hidden" onClick={()=>setOpen(true)}>☰</button>
 ...
 {open&&<div className="fixed inset-0 z-[100] md:hidden"><button aria-label="Close menu overlay" className="absolute inset-0 bg-slate-950/60" onClick={()=>setOpen(false)}/><aside className="absolute inset-y-0 left-0 flex w-[88vw] max-w-sm flex-col bg-white p-5 text-slate-900 shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 pb-5"><div className="flex items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-emerald-100 bg-emerald-50 text-sm font-black text-emerald-900">AM</div><div className="min-w-0"><div className="font-black text-emerald-950">AMQM</div><div className="mt-0.5 text-[10px] font-bold uppercase tracking-[.08em] text-slate-500">{me?.role==='teacher'?'Teacher Workspace':me?.role==='parent'?'Parent Portal':'School Management'}</div></div></div><button aria-label="Close navigation" className="mobile-close" onClick={()=>setOpen(false)}>×</button></div><nav className="mt-6 flex-1 space-y-2 overflow-y-auto">{links.map(([href,label])=><Link key={href} href={href} onClick={()=>setOpen(false)} className="mobile-nav-link"><span>{label}</span><span>→</span></Link>)}</nav><Link href="/" onClick={()=>setOpen(false)} className="mt-5 block rounded-2xl bg-[#06372f] px-4 py-3.5 text-center text-sm font-black text-white">View website</Link><button onClick={signOut} className="mt-2 block w-full rounded-2xl bg-slate-100 px-4 py-3.5 text-sm font-black text-emerald-950">Sign out</button></aside></div>}

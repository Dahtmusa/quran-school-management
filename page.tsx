    ...
    <main className="min-h-screen overflow-x-hidden bg-white text-slate-900">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex min-h-[78px] max-w-[1320px] items-center justify-between gap-5 px-5 sm:px-7">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" aria-label="Open navigation" aria-expanded={mobileNav} className="mobile-menu-button relative z-[10001] xl:hidden" onClick={() => setMobileNav(true)}>☰</button>
          </div>
        </div>
      </header>
      ...
      {mobileNav && (
        <div className="fixed inset-0 z-[9999] xl:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 z-[10000] flex w-[88vw] max-w-sm flex-col bg-white p-5 shadow-2xl" role="dialog" aria-modal="true" aria-label="Mobile navigation">
            ...
            <nav className="mt-6 flex-1 space-y-2 overflow-y-auto">
              {nav.map((x: any) => <Link key={x.label} href={x.href} aria-current={activeNav === x.href ? 'page' : undefined} onClick={() => { setActiveNav(x.href); setMobileNav(false); }} className={`mobile-nav-link ${activeNav === x.href ? 'active' : ''}`}><span>{x.label}</span><span>→</span></Link>)}
            </nav>

-- Migration 091: Auto-detect memorization direction and fix canonical start position
--
-- When a teacher submits a historical eval the function now:
--   1. Compares start vs end global_ayah to determine the true direction
--        end >= start  →  baqarah_to_nas  → canonical start = Surah 2  (Al-Baqarah), Ayah 1
--        end <  start  →  nas_to_baqarah  → canonical start = Surah 114 (An-Nas),    Ayah 1
--   2. Replaces whatever start the teacher entered with the canonical starting point
--   3. Keeps the end position (current ayah) exactly as submitted
--   4. Corrects the student's memorization_direction and start_surah/start_ayah in the DB
...
BEGIN
  ...
  -- Fetch end position first (this is always kept as-is)
  ...
  -- Fetch whatever start the teacher submitted — only used to detect direction
  ...
  -- Compare global positions: end >= start means forward (Baqarah→Nas)
  IF r_end.global_ayah >= r_start.global_ayah THEN
    v_direction := 'baqarah_to_nas';
    v_can_surah := 2;    -- Al-Baqarah
    v_can_ayah  := 1;
  ELSE
    v_direction := 'nas_to_baqarah';
    v_can_surah := 114;  -- An-Nas
    v_can_ayah  := 1;
  END IF;

  -- ── Override start with canonical starting position ──────────────────────
  SELECT * INTO r_start FROM public.quran_verses WHERE surah = v_can_surah AND ayah = v_can_ayah;
  IF r_start IS NULL THEN
    RAISE EXCEPTION 'Quran verse data missing for canonical start (surah % ayah 1)', v_can_surah;
  END IF;

  -- ── Correct student direction and start position ─────────────────────────
  ...
  INSERT INTO public.evaluations(
    ...
    v_can_surah, v_can_ayah, r_e1.surah, r_e1.ayah,
  ...
  -- Update student current position (keep end as-is, direction already fixed above)
  UPDATE public.students SET

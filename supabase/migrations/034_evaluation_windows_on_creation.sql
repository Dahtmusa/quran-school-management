-- Evaluation windows belong to the evaluation campaign itself, not the general school calendar.
-- Admin selects the operational term, evaluation number, opening and closing times, then classes.

DROP FUNCTION IF EXISTS public.create_evaluation_campaign(uuid,smallint,text,uuid,uuid[]);
...
BEGIN
  ...
  IF p_opens_at::date < v_term.starts_on OR p_closes_at::date > v_term.ends_on THEN
    RAISE EXCEPTION 'Evaluation window must fall within the selected academic term (% to %)', v_term.starts_on, v_term.ends_on;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Evaluation % already exists for this term. Use the existing campaign or edit its window.', p_evaluation_number;
  END IF;
  ...
  IF p_evaluation_number > 1 THEN
    ...
    IF missing_count > 0 THEN
      RAISE EXCEPTION 'Evaluation % cannot open yet: % student(s) in the selected classes still need Evaluation % approved.',
...
GRANT EXECUTE ON FUNCTION public.create_evaluation_campaign(uuid,smallint,text,timestamptz,timestamptz,uuid[]) TO authenticated;
REVOKE EXECUTE ON FUNCTION public.create_evaluation_campaign(uuid,smallint,text,timestamptz,timestamptz,uuid[]) FROM anon;

-- Evaluation windows no longer need to be represented as general school-calendar events.
-- Keep the historical rows for auditability, but prevent the general calendar UI from creating them.

-- Migration 060: Add eval3 mode to bulk_import_historical_evals.
--
-- Adds a p_mode parameter ('eval1_eval2' | 'eval3').
-- In eval3 mode the function inserts only evaluation_number=3 and updates
-- student.current to the Eval 3 end position so the term can be completed
-- and Second Term's Eval 1 automatically starts from there.

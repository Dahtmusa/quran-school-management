...
export async function updateMyProfile(input:{full_name:string;phone?:string|null;avatar_url?:string|null;employment_status?:string|null;job_title?:string|null;department?:string|null;joined_on?:string|null;id_expires_on?:string|null;bio?:string|null;show_on_website?:boolean;username?:string|null;qualifications?:string|null;experience?:string|null;subjects?:string|null;email?:string|null;preferred_email?:string|null}) {
  const { data, error } = await supabase().rpc('update_my_profile', {
    p_full_name: input.full_name,
    p_phone: input.phone || null,
    p_avatar_url: input.avatar_url || null,
    p_employment_status: input.employment_status || null,
    p_job_title: input.job_title || null,
    p_department: input.department || null,
    p_joined_on: input.joined_on || null,
    p_id_expires_on: input.id_expires_on || null,
    p_bio: input.bio || null,
    p_show_on_website: !!input.show_on_website,
    p_username: input.username || null,
    p_qualifications: input.qualifications || null,
    p_experience: input.experience || null,
    p_subjects: input.subjects || null,
    p_email: input.email || null,
    p_preferred_email: input.preferred_email || null,
  });
  if (error) throw error;
  return data;
}

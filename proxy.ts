...
const roleRoutes:{prefix:string;roles:string[]}[]=[
 ...
 {prefix:'/profile',roles:['super_admin','admin','principal']},
...
function dashboard(role:string){if(role==='teacher')return '/teacher';if(role==='parent')return '/parent';if(role==='security')return '/security';if(role==='finance')return '/fees';if(role==='admissions')return '/admissions/manage';return '/admin'}
